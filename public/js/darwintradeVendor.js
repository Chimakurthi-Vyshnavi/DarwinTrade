...
if (document.getElementsByClassName('thumbnail').length) {
    document.getElementsByClassName('thumbnail')[0].addEventListener('change', async function () {
        ...
        } else displayAlert('Invalid Image Format', `File ${file.name} is not a Valid Image.`, 'warning', ()=>{})
    });
}
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    if (xhr.status === 400) {
                        ...
                        displayAlert(response.error, 'Failed to Add Product.', 'error', ()=>{ location.reload() })
                    }  
                }
            });
        } catch (err) {
            displayAlert('Failed to Add Product', 'Please try again later.', 'warning', ()=>{ location.reload() })
        }
    })
}
    ...
    if (productid) {
        $.ajax({
            ...
            error: function (xhr, status, error) {
                if (xhr.status === 400) {
                    var response = JSON.parse(xhr.responseText);
                    displayAlert(response.error, 'Please try again later.','warning')
                }
            }
        })
    }
    ...
    if (productid) {
        try {
            ...
            $.ajax({
                ...
                contentType: false,
                success: function (response) {
                    displayAlert('Success','Changes Added to the Product Successfully.', 'success', ()=>{ location.reload() })
                },
                error: function (xhr, status, error) {
                    if (xhr.status === 400) {
                        var response = JSON.parse(xhr.responseText);
                        displayAlert(response.error, 'Failed to Edit Product.' ,'error', ()=>{ location.reload() })
                    } 
                }
            });
        } catch (err) {
            displayAlert('Failed to Edit Product','Please try again later.','warning', ()=>{ location.reload() })
        }
    }
    else displayAlert('Product Not Selected','','warning', ()=>{ location.reload() })
})
...
$("#accountForm").submit(function(e) {
    ...
    try {
        ...
        $.ajax({
            ...
            contentType: false,
            success: function (response) {
                displayAlert('Success','Account Updated Successfully.', 'success', ()=>{ location.reload() })
            },
            error: function (xhr, status, error) {
                if (xhr.status === 400) {
                    var response = JSON.parse(xhr.responseText);
...
if (verifyForm) {
    verifyForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (data.success) displayAlert('Success!', 'Your password has been changed.', 'success', () => { location.assign('/vendor'); });
            else if (data.error === "passwords don't match") confirmPasswordError.textContent = "Passwords didn't match. Try again."
            else if (data.error === "incorrect current password") passwordError.textContent = 'Please enter the correct password.'
            else if (data.error === 'Vendor Not Found') emailError.textContent = "Please enter the correct email."
        } catch (error) {
