...
if (vendorForm.length) {
    vendorForm[0].addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (data.error) {
                displayAlert(data.error, 'Failed to Add Vendor.', 'error', ()=>{})
            }
            else {
                $.ajax({
                    type: 'POST',
    ...
    if (vendor) {
        $.ajax({
            ...
            success: function (response) {
                ...
                rec.disabled = false
                rec.value = response.vendorAgreement.recurringCommission
            },
            error: function (xhr, status, error) {
                if (xhr.status === 400) {
                    var response = JSON.parse(xhr.responseText);
...
$("#adminAccountForm").submit(function(e) {
    ...
    try {
        ...
        $.ajax({
            ...
            contentType: false,
            success: function (response) {
                displayAlert('Success', response.message, 'success', ()=>{ location.reload() })
            },
            error: function (xhr, status, error) {
                if (xhr.status === 400) {
                    var response = JSON.parse(xhr.responseText);
                    displayAlert(response.error, 'Failed to Update Account.', 'error', ()=>{})
                } 
            }
        });
    } catch (err) {
        displayAlert('Failed to Update Account ', 'Please try again it later.', 'warning', ()=>{})
    }
})