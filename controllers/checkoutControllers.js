...
module.exports.postMail = async (req, res) => {
    try {
        ...
        if (total > employeeTotalPoints)
            ...
            await employee.save();
            await Promise.all([
                // reduceEmployeePoints(employeeId, total),
...
// async function reduceEmployeePoints(employeeId, totalPoints) {
//     try {
//         const employee = await Employee.findOne({ _id: employeeId });
//         if (!employee) throw new Error('Employee not found');
...
//         await employee.save();
//     } catch (error) {
//         throw new Error('Error reducing employee points: ' + error.message);
//     }
// }
