  ...
  try {
    ...
    if(req.files.thumbnail) {
      ...
      const product = await ProductSchema.dummyProduct.create({
          title: req.body.title.trim(), description: req.body.shortdescription.trim(), price: req.body.price.trim(), stock: req.body.price.trim(), 
          ...
          id: count+1, category:req.body.category, mainCategory: mainCategory._id, brand: req.body.brand.trim(), publishedAt: date.toISOString(), 
          minStock:req.body.minStock ,rating:3
        })
      io.emit('new-product', { 
        message: `New product in category "${product.category}" has been added!`
      });
...
module.exports.postChangePassword = async (req, res) => {
  try {
    ...
    const err = handleErrors(error)
    res.status(400).json({ success: false, error: err || 'Failed to change password' });
  }
}
